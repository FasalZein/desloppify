import { afterEach, describe, expect, test } from "bun:test";
import { mkdtempSync, mkdirSync, rmSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";

const tempRoots: string[] = [];
const cliPath = join(process.cwd(), "src/cli.ts");

afterEach(() => {
  for (const root of tempRoots.splice(0)) rmSync(root, { recursive: true, force: true });
});

describe("scan command go pack", () => {
  test("scan --pack go emits go findings only", () => {
    const root = mkdtempSync(join(tmpdir(), "desloppify-scan-go-"));
    tempRoots.push(root);
    mkdirSync(join(root, "internal"), { recursive: true });
    mkdirSync(join(root, "src"), { recursive: true });
    writeFileSync(join(root, "go.mod"), "module example.com/demo\n\ngo 1.22\n");
    writeFileSync(join(root, "internal", "main.go"), "package internal\nfunc run(err error) {\n  panic(err)\n  if err.Error() == \"boom\" {\n    return\n  }\n}\n");
    writeFileSync(join(root, "src", "view.tsx"), "useEffect(async () => { await load(); }, []);\n");

    const result = Bun.spawnSync(["bun", cliPath, "scan", root, "--pack", "go", "--json"], {
      cwd: process.cwd(),
      stdout: "pipe",
      stderr: "pipe",
    });
    const output = JSON.parse(result.stdout.toString()) as { findings: Array<{ rule_id: string }> };

    expect(result.exitCode).toBe(1);
    expect(output.findings.map((finding) => finding.rule_id)).toEqual(expect.arrayContaining([
      "PANIC_CALL_GO",
      "ERROR_STRING_COMPARE_GO",
    ]));
    expect(output.findings.map((finding) => finding.rule_id)).not.toContain("USEEFFECT_ASYNC");
  });
});
