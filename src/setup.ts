...
export function getHooksInstallCommand(): { command: string; args: string[]; display: string } {
  ...
  const display = [
    ...
    '  printf "%s\\n" "desloppify install-hooks must run inside a git repository" >&2',
    '  exit 1',
    '}',
    ...
    'if [ -n "$normalized_hooks_path" ]; then',
    '  while [ "$normalized_hooks_path" != "/" ] && [ "${normalized_hooks_path%/}" != "$normalized_hooks_path" ]; do',
    '    normalized_hooks_path="${normalized_hooks_path%/}"',
    '  done',
    'fi',
    'managed_hooks_path=$(cd "$repo_root" && pwd)/.githooks',
    'default_hooks_path=$(cd "$repo_root" && pwd)/.git/hooks',
    'if [ -n "$normalized_hooks_path" ] && [ "$normalized_hooks_path" != ".githooks" ] && [ "$normalized_hooks_path" != "./.githooks" ] && [ "$normalized_hooks_path" != ".git/hooks" ] && [ "$normalized_hooks_path" != "./.git/hooks" ] && [ "$normalized_hooks_path" != "$managed_hooks_path" ] && [ "$normalized_hooks_path" != "$default_hooks_path" ]; then',
    '  printf "%s\\n" "Refusing to replace existing core.hooksPath=$current_hooks_path" >&2',
    '  exit 1',
    'fi',
    'check_active_hooks=1',
    'active_hooks_dir="$default_hooks_path"',
    'if [ "$normalized_hooks_path" = ".githooks" ] || [ "$normalized_hooks_path" = "./.githooks" ] || [ "$normalized_hooks_path" = "$managed_hooks_path" ]; then',
    '  check_active_hooks=0',
    'fi',
    'if [ "$check_active_hooks" -eq 1 ]; then',
