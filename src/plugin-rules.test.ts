...
describe("plugin rules", () => {
  test("loads plugin rules from config-declared local files", () => {
    ...
    writeFileSync(join(tempRoot, "plugin.cjs"), `module.exports = { rules: [{ id: 'contains-acme', category: 'ai-slop', severity: 'MEDIUM', message: 'Contains ACME', description: 'Contains ACME marker', pattern: 'ACME' }] };`);
  ...
  test("resolves plugin preset configs from extends refs", () => {
    ...
    writeFileSync(join(tempRoot, "plugin.cjs"), `module.exports = { configs: { recommended: { rules: { CONSOLE_LOG: { enabled: false } } } } };`);
