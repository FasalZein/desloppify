...
export function runFileMetricsFromEntries(entries: FileEntry[], options: FileMetricsOptions = {}): Issue[] {
  ...
  for (const entry of entries) {
    ...
    if (architecture) {
      limits = isRouteFile && !isRouteRegistrarIndex ? architecture.loc.routeSlice : architecture.loc.default;
    } else if (isTestFile || isScript) {
      limits = TEST_LIMITS;
    } else if (isUiFile || isRouteFile) {
      limits = UI_LIMITS;
    }

    const isGenFile = /\.gen\.|\.generated\.|payload-types|\.d\.ts$/.test(filePath);
    if (loc >= limits.god && !isGenFile) {
      issues.push({
        id: "GOD_FILE",
        category: "complexity",
        severity: "CRITICAL",
    ...
    } else if (loc >= limits.hard && !isGenFile) {
      issues.push({
        id: "LARGE_FILE",
        category: "complexity",
        severity: "HIGH",
        ...
        file: filePath,
        line: 1,
        message: `File has ${loc} lines of code — approaching god file territory (threshold: ${limits.hard})`,
        tool: "file-metrics",
      });
    } else if (loc >= limits.soft && !isGenFile) {
      issues.push({
        id: "LONG_FILE",
        category: "complexity",
        severity: "MEDIUM",
    ...
    const hasStarReexport = lines.some((l) => /^\s*export\s+\*\s+from\s+/.test(l));
