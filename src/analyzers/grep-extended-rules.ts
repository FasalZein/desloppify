...
export const GREP_EXTENDED_RULES: GrepExtendedRuleDefinition[] = [
  ...
  },
  {
    ...
    severity: "HIGH",
    tier: 0,
    message: "dangerouslySetInnerHTML — sanitize untrusted HTML or avoid raw injection entirely",
    desc: "dangerouslySetInnerHTML — sanitize or avoid raw HTML",
    fileFilter: /\.(tsx|jsx)$/,
  },
  {
    id: "INTERACTIVE_DIV",
  ...
  },
  {
    ...
    severity: "HIGH",
    tier: 0,
    message: "yaml.load without SafeLoader — unsafe deserialization risk. Use yaml.safe_load.",
    desc: "yaml.load without SafeLoader",
    fileFilter: /\.py$/,
  },
  {
    id: "SUBPROCESS_SHELL_TRUE",
    pattern: /subprocess\.(run|Popen|call|check_output|check_call)\([^\n]*shell\s*=\s*True/,
    category: "security-slop",
    severity: "HIGH",
    tier: 0,
    message: "subprocess with shell=True — command injection risk. Pass argv list instead.",
    desc: "subprocess shell=True — command injection risk",
    fileFilter: /\.py$/,
  },
  {
    id: "MUTABLE_DEFAULT",
  ...
  },
  {
    ...
    severity: "HIGH",
    tier: 0,
    message: "panic() in Go code — return errors explicitly unless this is truly unrecoverable",
    desc: "panic() in Go code — prefer explicit error returns",
    fileFilter: /\.go$/,
  },
  {
    id: "ERROR_STRING_COMPARE_GO",
    pattern: /\.Error\(\)\s*(==|!=)/,
    category: "defensive-programming",
    severity: "MEDIUM",
    tier: 0,
    message: "Comparing err.Error() strings is brittle — prefer sentinel errors or errors.Is / errors.As",
    desc: "Compare Go errors structurally, not by string",
    fileFilter: /\.go$/,
  },
  {
    id: "GO_TEST_SKIP",
    pattern: /\b(?:t|suite)\.Skip(?:Now|f)?\s*\(/,
    category: "test-quality",
    severity: "LOW",
    tier: 0,
    message: "Skipped Go test — resolve or remove it instead of leaving it disabled",
    desc: "Skipped Go test — resolve or remove",
    fileFilter: /(_test\.go|\.go)$/,
  },
  {
    id: "RUBY_PUTS_DEBUG",
    pattern: /^\s*puts\s+/,
    ...
    severity: "LOW",
    tier: 1,
    message: "puts debugging in Ruby code — remove it or switch to structured logging",
    desc: "Ruby puts debugging — remove it",
    fix: "Remove the puts statement or replace it with logger usage",
    fileFilter: /\.rb$/,
  },
  {
    id: "RUBY_BARE_RESCUE",
    ...
    severity: "HIGH",
    tier: 0,
    message: "Bare rescue in Ruby catches StandardError implicitly — rescue a specific exception type",
    desc: "Bare Ruby rescue — catch specific exception types",
    fileFilter: /\.rb$/,
  },
  {
    id: "RUBY_TEST_SKIP",
    pattern: /\b(skip\s+["']|pending\s*["'])/,
    category: "test-quality",
    severity: "LOW",
    tier: 0,
    message: "Skipped Ruby test — resolve or remove it instead of leaving it pending",
    desc: "Skipped Ruby test — resolve or remove",
    fileFilter: /(_test\.rb|_spec\.rb)$/,
  },
  {
